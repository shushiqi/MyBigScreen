  ...
  export default {
    ...
    methods: {
      ...
      count(timestamp) {
        ...
        // console.log(this.displayValue, typeof this.displayValue);
      ...
      formatNumber(num) {
        const index = String(num).indexOf(".") + 1;
        // 数字大于1000且没有小数点时,添加分隔符
        if (index <= 0) {
          const rgx = /(\d+)(\d{3})/;
          if (this.separator && !this.isNumber(this.separator)) {
            while (rgx.test(num)) {
              console.log(typeof num);
              num = String(num).replace(rgx, "$1" + this.separator + "$2");
            }
          }
          return num;
        }
